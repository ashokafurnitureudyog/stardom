...
const MenuLink: React.FC<MenuLinkProps> = ({ item, index, totalItems = 0, isMobile = false }) => {
    const getLinkColor = () => {
    ...
    return (
        <Link
            ...
            className={`
                relative
                ${isMobile ? 'w-full text-lg py-4 px-6 hover:bg-default-100 rounded-lg transition-all duration-300' : 'group'}
                ${!isMobile ? 'after:content-[""] after:absolute after:w-0 after:h-0.5 after:bg-primary/80 after:left-0 after:right-0 after:-bottom-1 after:mx-auto after:transition-all after:duration-300 hover:after:w-full' : ''}
